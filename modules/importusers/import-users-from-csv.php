...
add_action('plugins_loaded', array('SMSify_IS_IU_Import_Users', 'init'), 201);
 ...
 * @since 0.1
 **/
class SMSify_IS_IU_Import_Users {
	...
	public function init() {
		...
		add_action('init', array( __CLASS__, 'register_user_taxonomy'), 9);
...
<?php
			...
			if ( is_wp_error( $user_id ) ) {
				...
				if ( $usermeta ) {
					foreach ( $usermeta as $metakey => $metavalue ) {
    					...
    					//MT: Handle User Groups differently from meta    
    					if($metakey == "user-group") {
    					    self::save_user_user_groups($user_id, $metavalue);
    					} else {
	...
	public static function register_user_taxonomy() {
        register_taxonomy('user-group', 'user');
    }

    private static function save_user_user_groups( $user_id, $user_group) {
        ...
        if(is_null($user_group) || empty($user_group)) {
            wp_delete_object_term_relationships( $user_id, 'user-group' );
        } else {
            /* Sets the terms (we're just using a single term) for the user. */
            wp_set_object_terms( $user_id, $user_group, 'user-group');
        }
